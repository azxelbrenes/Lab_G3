...
export class TipoCedulaFormPage implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (idParam) {
      ...
      this.isEdit = true;
      this.form.get('tipoCedula1')?.disable();
      this.cargar(this.recordId);
    }
  }

  cargar(id: number): void {
    this.tipoCedulaService.obtenerPorId(id).subscribe({
      next: (resp) => {
  ...
  async guardar(): Promise<void> {
    ...
    const request$ = this.isEdit
      ? this.tipoCedulaService.actualizarTipoCedula(payload)
      : this.tipoCedulaService.insertarTipoCedula(payload);
    ...
    request$.subscribe({
      next: async () => {
        ...
        await toast.present();
        this.router.navigateByUrl('/tipos-cedula');
      },
      error: async () => {
        this.guardando = false;
        const toast = await this.toastCtrl.create({
